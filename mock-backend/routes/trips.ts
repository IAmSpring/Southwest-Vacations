...
import { Trip, TripDetail, Booking } from '../../src/sharedTypes.js';
...
router.get('/', (req: Request, res: Response) => {
  try {
    ...
    let trips = db.get('trips').value() as TripDetail[];
    ...
    if (destination) {
      ...
      trips = trips.filter(trip => trip.destination.toLowerCase().includes(searchTerm));
    }
    ...
    if (availability) {
      ...
      trips = trips.filter(trip => trip.datesAvailable && trip.datesAvailable.includes(dateStr));
    }
    ...
    const tripsForDisplay = trips.map(({ datesAvailable, ...rest }: TripDetail) => ({
      ...rest,
...
router.get('/search', (req: Request, res: Response) => {
  try {
    ...
    if (minPrice) {
      filteredTrips = filteredTrips.filter(trip => trip.price >= Number(minPrice));
    }

    if (maxPrice) {
      filteredTrips = filteredTrips.filter(trip => trip.price <= Number(maxPrice));
    }
    ...
    if (startDate) {
      filteredTrips = filteredTrips.filter(
        trip => trip.datesAvailable && trip.datesAvailable.some(date => date >= String(startDate))
      );
    }

    if (endDate) {
      filteredTrips = filteredTrips.filter(
        trip => trip.datesAvailable && trip.datesAvailable.some(date => date <= String(endDate))
      );
    }
...
router.get('/:tripId', (req: Request, res: Response) => {
  try {
    ...
    const trip = db.get('trips').find({ id: req.params.tripId }).value() as TripDetail;
    ...
    if (includeBookingStats === 'true') {
      ...
      const bookings = db.get('bookings').filter({ tripId: trip.id }).value() as Booking[];
...
router.put('/:tripId', (req: Request, res: Response) => {
  try {
    ...
    const existingTrip = db.get('trips').find({ id: tripId }).value();
    ...
    // Update trip
    db.get('trips').find({ id: tripId }).assign(tripData).write();

    // Get updated trip
    const updatedTrip = db.get('trips').find({ id: tripId }).value();
